# -*- coding: utf-8 -*-
"""
Created on Thu Feb 12 13:20:09 2026
...
#%% New colourmap
color_mapping7 =  color_mapping2    
#%% Random forest

#%% Dividing into the different experiments
...
feature_name_mapping = {
    ...
    'mean_Open_natural_areas' : r"$V_{\mathrm{F(LowV)}}$", 
...
#%% The model that has the least overfitting (equal to or less than 0.1)
low_overfit = all_results_df_10[all_results_df_10['train_acc']- all_results_df_10['test_acc']  <= 0.10]
#%% The find the top perfomes based on the 90% quantile
top_models = low_overfit[low_overfit['test_acc'] >= low_overfit['test_acc'].quantile(0.90)]
#%%
import ast

params_df = top_models['params'].apply(ast.literal_eval).apply(pd.Series)
#%%
#params_df = top_models['params'].apply(pd.Series)
#%% Within th best perfoming model, take the median of the parameters
best_params = {
    'n_estimators': int(params_df['n_estimators'].median()),
    'max_depth': int(params_df['max_depth'].median()),
    'min_samples_split': int(params_df['min_samples_split'].median()),
    'min_samples_leaf': int(params_df['min_samples_leaf'].median())
    #'max_features': params_df['max_features'].median()
}
...
# Climate & Landscape
X = climate_landscape
y = hydro_sing_clustered['C_k10_AllS_NOClimate_remapped'] 
...
recalls_0 = []
f1s_0 = []
cumulative_cm_0 = np.zeros((10, 10), dtype=int) # For the cumulative confusion matrix #CHANGE HERE
fold_cms_0 = []
#for the ROC curves
n_classes = 10  # clusters #CHANGE HERE
...
for fold, (train_index, test_index) in enumerate(kf.split(X, y)):
    ...
    # Predict and evaluate on test set
      ...
      # Get predicted probabilities for ROC
        ...
        cm = confusion_matrix(y_test, y_test_pred, labels=np.arange(10)) #CHANGE HERE
...
#Morans I
residuals = (y_pred_full_0.loc[y.index] != y).astype(int)
coords_array = hydro_sing_clustered[['lon_snap','lat_snap']].values

w = KNN.from_array(coords_array, k=8)
w.transform = 'r'
...
recalls_2 = []
f1s_2 = []
cumulative_cm_2 = np.zeros((10, 10), dtype=int) # For the cumulative confusion matrix #CHANGE HERE
fold_cms_2 = []
#for the ROC curves
n_classes = 10  # clusters #CHANGE HERE
...
#Morans I
residuals = (y_pred_full_2.loc[y.index] != y).astype(int)
coords_array = hydro_sing_clustered[['lon_snap','lat_snap']].values
...
y = hydro_sing_clustered['C_k10_AllS_NOClimate_remapped']    # CHANGE HERE
...
recalls_3 = []
f1s_3 = []
cumulative_cm_3 = np.zeros((10, 10), dtype=int)   # CHANGE HERE
...
#Morans I
residuals = (y_pred_full_3.loc[y.index] != y).astype(int)
coords_array = hydro_sing_clustered[['lon_snap','lat_snap']].values

w = KNN.from_array(coords_array, k=8)  # Why k=8??
w.transform = 'r'
...
recalls_4  = []
f1s_4  = []
cumulative_cm_4  = np.zeros((10, 10), dtype=int) # For the cumulative confusion matrix #CHANGE HERE
fold_cms_4 = []
#for the ROC curves
n_classes = 10  # clusters #CHANGE HERE
...
#Morans I
residuals = (y_pred_full_4.loc[y.index] != y).astype(int)    # CHNGE HERE
coords_array = hydro_sing_clustered[['lon_snap','lat_snap']].values
...
recalls_5 = []
f1s_5 = []
cumulative_cm_5 = np.zeros((10, 10), dtype=int) # For the cumulative confusion matrix   #CHANGE HERE
fold_cms_5 = []
#for the ROC curves
n_classes = 10  # clusters   #CHANGE HERE
...
#Morans I
residuals = (y_pred_full_5.loc[y.index] != y).astype(int)    # CHNGE HERE
coords_array = hydro_sing_clustered[['lon_snap','lat_snap']].values
...
recalls_6  = []
f1s_6  = []
cumulative_cm_6  = np.zeros((10, 10), dtype=int) # For the cumulative confusion matrix   #CHANGE HERE
fold_cms_6 = []
...
#Morans I
residuals = (y_pred_full_6.loc[y.index] != y).astype(int)    # CHNGE HERE
coords_array = hydro_sing_clustered[['lon_snap','lat_snap']].values
...
for i, (df, ax) in enumerate(zip(importance_dfs, axes)):
    ...
    #ax.errorbar(df_display['Median Importance'], y_positions, 
    #xerr=[np.minimum(df_display['Std Importance'], df_display['Median Importance']), df_display['Std Importance']], fmt='.', color='black')
...
for j in range(len(importance_dfs), len(axes)):
    fig.delaxes(axes[j])
    
plt.tight_layout()
plt.savefig(r'U:\Hydrological Data\EStreams\FeatureImportance_allversions.png',bbox_inches='tight',dpi=300)
...
for i, (ax, cm, title) in enumerate(zip(axes.ravel(), cms, titles)):
    ...
    for (row, col), val in np.ndenumerate(cm):
        ...
        text_color = "white" if cm_norm[row, col] > 0.5 else "black"
...
for j in range(len(cms), len(axes_flat)):
    fig.delaxes(axes_flat[j])
    
plt.subplots_adjust(wspace=-0.2, hspace=0.275)
#plt.tight_layout() 
plt.savefig(r'U:\Hydrological Data\EStreams\\ConfusionMatrix_allversions.png',bbox_inches='tight',dpi=300)


#%% CM test with baseline random guess added
exp_colors = [
    "#159895",  # teal (lightseagreen adjusted; slightly bluer)
    "#1f77b4",  # tab:blue (unchanged)
    "#2E8B57",  # sea green (for darker vegetation; less red)
...
cms = [cm_avg_0, cm_avg_2, cm_avg_3, cm_avg_4, cm_avg_5, cm_avg_6] 
# experiment titles 
titles = [ "1-CL", "2-C", "3-VLC", "4-VLS", "5-SGT", "6-A" ] 
letters = list(string.ascii_lowercase) 
cumulative_cms = [cumulative_cm_0,cumulative_cm_2,cumulative_cm_3,cumulative_cm_4,cumulative_cm_5,cumulative_cm_6] 
cm_counts = cumulative_cms[0] # how do i calculate for all? Or is that not needed?? 
class_counts = np.sum(cm_counts, axis=1) 
total = np.sum(cm_counts) 
random_baseline = class_counts / total * 100 


fig, axes = plt.subplots(nrows=4, ncols=2, figsize=(14, 20), gridspec_kw={'height_ratios': [1, 1, 1, 0.5]}) 
#plt.subplots_adjust(wspace=0.01, hspace=0.3) 
for i, (ax, cm, title) in enumerate(zip(axes.ravel(), cms, titles)):
...
# --- BASELINE PLOT (BOTTOM ROW) ---
# Remove the last empty subplot (bottom right) 
fig.delaxes(axes[3, 1]) # Make bottom-left span full width 
gs = axes[3, 0].get_gridspec() 
axes[3, 0].remove()
ax_baseline = fig.add_subplot(gs[3, :]) 


pos = ax_baseline.get_position()

new_width = pos.width * 0.6  # e.g. 60% of original width
new_x0 = pos.x0 + (pos.width - new_width) / 2

ax_baseline.set_position([
    new_x0,
    pos.y0,
    new_width,
    pos.height
...
classes = np.arange(1, len(random_baseline) + 1) 
bars = ax_baseline.bar(classes, random_baseline, color='lightgrey', edgecolor='black') # Labels on bars 
for x, val in zip(classes, random_baseline): ax_baseline.text(x, val + 0.5, f"{val:.0f}%", ha='center', fontsize=12) 
ax_baseline.set_xticks(classes) 
ax_baseline.set_xticklabels(classes, fontsize=14) 
ax_baseline.tick_params(axis='y', labelsize=14) 
ax_baseline.set_ylabel("Random baseline (%)", fontsize=14) 
ax_baseline.set_xlabel("HRT", fontsize=14) 
ax_baseline.set_title("(g) Random guess baseline", fontsize=16, loc='left') 
ax_baseline.set_ylim(0, max(random_baseline) * 1.2) 
plt.subplots_adjust(wspace=-0.2, hspace=0.275)

 

# NOW manually resize & center the baseline axis
pos = ax_baseline.get_position()

new_width = pos.width * 0.75   # e.g. 50% of original width
new_x0 = pos.x0 + (pos.width - new_width) / 2
...
#plt.tight_layout()
plt.savefig(r'U:\Hydrological Data\EStreams\ConfusionMatrix_baseline.png',bbox_inches='tight',dpi=300)
...
for i in range(n_clusters):
    ...
    diag_values = {exp: cm[i, i] for exp, cm in cms.items()}
...
catchment_clusters = hydro_sing_clustered['C_k10_AllS_NOClimate_remapped']  # CHNAGE HERE!!!!!!!
...
max_diff = catchment_diff.max()
bins = [0, 1, 3, 6, max_diff + 1e-6]  # bin edges
...
hydro_sing_clustered['best_exp'] = catchment_best_exp
hydro_sing_clustered['plot_color'] = catchment_color
hydro_sing_clustered['plot_order'] = hydro_sing_clustered['best_exp'].apply(lambda x: exp_plot_order.index(x))
hydro_sign_sorted = hydro_sing_clustered.sort_values('plot_order')
...
present_exps = [exp for exp in exp_plot_order if exp in hydro_sing_clustered['best_exp'].unique()]
...
present_exps = [ 'exp_2','exp_3', 'exp_5']   ## CHANGE HERE!!!
...
plt.savefig(r'U:\Hydrological Data\EStreams\ConfusionMatrix_DRIVERS_temp_allversions_10C.png',bbox_inches='tight',dpi=300)
...
# List of RF run data for convenience
...
n_plots = len(all_y_test_list)

for idx in range(n_plots):
    ax = axes[idx]

    y_test_all = np.concatenate(all_y_test_list[idx]).ravel()
    y_pred_all = np.concatenate(all_y_pred_list[idx]).ravel()
    ...
    for i in range(n_classes):
        ...
        ax.plot(mean_fpr, mean_tpr, lw=2, color=color_mapping7[i],
                label=f'HRT {i+1} (AUC = {mean_auc:.2f} ± {std_auc:.2f})')
        ax.fill_between(mean_fpr, mean_tpr - std_tpr, mean_tpr + std_tpr,
                        color=color_mapping7[i], alpha=0.2)
        ...
        dot_points.append((fpr_point, tpr_point, color_mapping7[i]))
...
for j in range(n_plots, len(axes)):
    fig.delaxes(axes[j])
plt.tight_layout()
plt.savefig(r'U:\Hydrological Data\EStreams\ROC_PER_CLUSTER_allversions_10C.png',bbox_inches='tight',dpi=300)


#%% ROC curve with shaded area
all_runs = {
    0: (all_y_test_0, all_y_score_0, all_y_pred_0),
    2: (all_y_test_2, all_y_score_2, all_y_pred_2),
    3: (all_y_test_3, all_y_score_3, all_y_pred_3),
    4: (all_y_test_4, all_y_score_4, all_y_pred_4),
    5: (all_y_test_5, all_y_score_5, all_y_pred_5),
    6: (all_y_test_6, all_y_score_6, all_y_pred_6)
}
...
plt.grid(True)
plt.tight_layout()
plt.savefig(r'U:\Hydrological Data\EStreams\ROC_curve_allversions_10C.png',bbox_inches='tight', dpi=300)
...
plt.tight_layout()
plt.savefig(r'U:\Hydrological Data\EStreams\ROC_shade.png',bbox_inches='tight', dpi=300)
...
ax1.plot([0, 1], [0, 1], 'k--', lw=1.5, zorder=3)
...
plt.savefig(r'U:\Hydrological Data\EStreams\RF_TestingvsTraining_ROCCRUVE_10C.png',bbox_inches='tight', dpi=300)
...
plt.tight_layout()
plt.savefig(r'U:\Hydrological Data\EStreams\RF_TestingACC_perfold_10C.png',bbox_inches='tight', dpi=300)
...
y_pred = y_pred_full_0  # change depedning on the cluster you want to plot 0, 2,3,4,5,6 (only change here)
y_pred = y_pred_full_0.astype(int) # change depedning on the cluster you want to plot 0, 2,3,4,5,6 (only change here)
...
plt.savefig(r'U:\Hydrological Data\EStreams\map_CL_correct_incorrect_1CL.pdf',bbox_inches='tight', dpi=300)
plt.savefig(r'U:\Hydrological Data\EStreams\map_CL_correct_incorrect_1CL.png',bbox_inches='tight', dpi=300)
...
plt.tight_layout()
plt.savefig(r'U:\Hydrological Data\EStreams\FI_rankingscatter_v2_notext_10C.pdf',bbox_inches='tight', dpi=300)
plt.savefig(r'U:\Hydrological Data\EStreams\FI_rankingscatter_v2_notext_10C.png',bbox_inches='tight', dpi=300)
