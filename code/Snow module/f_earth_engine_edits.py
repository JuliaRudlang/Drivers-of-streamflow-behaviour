...
def elevation_stats(catch_id,work_dir):
    DEM = ee.Image("MERIT/DEM/v1_0_3") #here we load a DEM - here use MERIT DEM
    ele = DEM.select('dem') # select the elevation band
    ...
    shape_dir = Path(f'{work_dir}') #directory with shapefiles
    ...
    df.to_csv(f'U:/Hydrological Data/EStreams/Subset_v2_290324/Subset_240424/Snow_Module/output/elevation/stats/ele_{catch_id}.csv')
    dfs.to_csv(f'U:/Hydrological Data/EStreams/Subset_v2_290324/Subset_240424/Snow_Module/output/elevation/stats/slope_{catch_id}.csv')
    
    
def elevation_zones(catch_id,work_dir):# run first this section for one single catchment to test if it works, after run in parallel
    # get elevation image
    # DEM = ee.Image('CGIAR/SRTM90_V4')
    # ele = DEM.select('elevation')
    DEM = ee.Image("MERIT/DEM/v1_0_3") #here we load a DEM - here use MERIT DEM
    ele = DEM.select('dem') 
    ele_res = ele.resample('bilinear').reproject(crs=ele.projection().crs(), scale=100) #resample to lower resolution to speed up processing
    
    # make feature from catchment shape
    shape_dir = Path(f'{work_dir}')
    f = glob.glob(f'{shape_dir}/{catch_id}.shp')[0]
    shapefile = gpd.read_file(f)
    features = []
    for j in range(shapefile.shape[0]):
    ...
    df_el.to_csv(f'U:/Hydrological Data/EStreams/Subset_v2_290324/Subset_240424/Snow_Module/output/elevation/el_zones/{catch_id}.csv')
...
def run_function_parallel_elevation_zones(
    catch_list=list,
    work_dir_list=list,
    # threads=None # using whole coputer capacity
    threads=100
    ):
    # Set number of threads (cores) used for parallel run and map threads
    if threads is None:
